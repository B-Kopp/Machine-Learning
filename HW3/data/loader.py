...
class Contraceptive(DataLoader):
    def __init__(self, path='data/contraceptive.csv', verbose=False, seed=1):
...
class RedWine(DataLoader):
    def __init__(self, path='winequality-red.csv', verbose=False, seed=1):
        super().__init__(path, verbose, seed)

    def _load_data(self):
        self._data = pd.read_csv(self._path, header=1, delimiter=';')
