...
if __name__ == '__main__':
    ...
    if args.dim or args.skiprerun:
        if not args.ica and not args.pca and not args.rf and not args.rp:
    ...
    datasets = []
    dataset1_details = {
            'data': loader.RedWine(verbose=verbose, seed=seed),
            'name': 'red_wine',
    ...
    dataset2_details = {
            'data': loader.Contraceptive(verbose=verbose, seed=seed),
            'name': 'contra',
            'readable_name': 'Contraceptive',
            'best_nn_params': {'NN__activation': ['logistic'], 'NN__alpha': [0.0115],
                               'NN__hidden_layer_sizes': [(10, 10)], 'NN__learning_rate_init': [0.001]}
    }
